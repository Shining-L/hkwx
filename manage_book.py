...
@app.cli.command('update_cate_icon')
...
@app.cli.command('add_test_keyword')
...
@app.cli.command('update_cate_name')
...
@app.cli.command()
@click.option('-c', '--channel', default='', help='指定渠道')
def update_book(channel):
    ...
    channel_class = getattr(getattr(__import__('script.%s_book' % channel), '%s_book' % channel),
                            '%sBookSpider' % channel.capitalize())
