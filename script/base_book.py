...
class BookUpdater():
    ...
    def get_local_book_latest(self):
        ...
        query = text("""
            ...
            WHERE b.source = :channel_name
            GROUP BY b.book_id
        """)

        # 通过params参数传递channel_name，避免字符串拼接导致的SQL注入风险
        local_latest_books = db.session.execute(query, {"channel_name": self.spider.CHANNEL_NAME}).fetchall()
        print('查询时间', time.time() - t1)

        # 处理查询结果
        local_latest_books_dict = {
            b.channel_book_id: {
                "max_vid": int(b.volume_id or 0),
                "max_cid": int(b.chapter_id or 0),
                "book_id": int(b.book_id),
                "update_time": str(b.update_time),  # 直接使用str()转换时间对象
                "word_count": int(b.word_count),
                "chapter_num": int(b.chapter_num)
            } for b in local_latest_books
        }
        print("local_latest_books: ", local_latest_books_dict)
        return local_latest_books_dict
